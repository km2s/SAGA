          ...
          {skills.map(a => (
            <div key={a.id} className="flex items-center justify-between px-3 py-2 rounded"
              ...
              <div className="flex items-center gap-3">
                <NumericAttr attr={a} characterId={characterId} canEdit={canEdit} onSaved={onRefresh} />
                <span className="text-xs text-saga-dim">{Math.floor(a.value / 2)}</span>
                <span className="text-xs text-saga-dim">{Math.floor(a.value / 5)}</span>
              </div>
            </div>
          ))}
          {skills.length === 0 && <p className="text-sm text-saga-dim text-center py-6">Nenhuma perícia cadastrada.</p>}
        </div>
      </div>
    </div>
  )
...
function VinculosTab({ textFields, characterId, canEdit, onRefresh }: {
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-saga-dim leading-relaxed">
      ...
      {bonds.map(i => {
        ...
        return (
          <div key={i} className="rounded p-4" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            {canEdit
              ? <input type="text" defaultValue={nameField?.value ?? ''} onBlur={e => void saveName(e.target.value)}
                  ...
                  className="w-full bg-surface-2 border border-border rounded px-2 py-1.5 text-sm focus:outline-none text-saga-text mb-3" />
              : <p className="text-sm text-saga-text mb-3">{nameField?.value || <span className="text-saga-dim italic">—</span>}</p>
            }
            <div className="flex items-center gap-2">
              <span className="text-xs text-saga-dim">Força:</span>
                ...
                {Array.from({ length: MAX_SCORE }).map((_, j) => (
                  <button key={j} onClick={() => canEdit && void saveScore(j < score ? j : j + 1)}
                    ...
                    style={{
                      ...
                      borderColor: j < score ? ACCENT : 'rgba(255,255,255,0.15)',
    ...
    <div className="space-y-4">
      {san && (
        <div className="rounded p-4" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)' }}>
          <p className="font-almendra text-[9px] uppercase tracking-widest text-saga-dim mb-3">Sanidade</p>
          <div className="flex items-center gap-4 mb-2">
            ...
            <span className="text-xs text-saga-dim">/ 99</span>
          </div>
          {bp !== null && <p className="text-xs text-saga-dim">Ponto de Ruptura: <span className="font-cinzel font-bold" style={{ color: '#ef4444' }}>{bp}</span></p>}
        </div>
      )}
