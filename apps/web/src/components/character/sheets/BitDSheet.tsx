      ...
      {[0, 1, 2, 3].map(i => (
        <button key={i} onClick={() => void click(i)} disabled={!canEdit}
          ...
          style={{
            ...
            borderColor: i < attr.value ? color : 'rgba(255,255,255,0.2)',
  ...
  return (
    <button onClick={() => void toggle()} disabled={!canEdit}
      ...
      style={{
        ...
        borderColor: checked ? '#c9a22a' : 'rgba(255,255,255,0.2)',
        cursor: canEdit ? 'pointer' : 'default',
      }} />
  )
}
        ...
        return (
          <div key={groupName}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[10px] text-saga-dim">{groupRating} dado{groupRating !== 1 ? 's' : ''} de atributo</span>
              <div className="flex-1 h-px" style={{ background: `${color}20` }} />
              <span className="text-[9px] text-saga-dim/50">XP</span>
            </div>
              ...
              {actions.map(action => {
                ...
                return (
                  <div key={action.key} className="flex items-center gap-3 py-2 px-1">
                    ...
                    <span className="flex-1 text-sm text-saga-text">{action.label}</span>
                    {attr
                        ...
                        <div className="flex gap-[5px] items-center opacity-30">
                          {[0, 1, 2, 3].map(i => (
                            <div key={i} className="rounded-full border flex-shrink-0"
                              style={{ width: 11, height: 11, borderColor: 'rgba(255,255,255,0.2)' }} />
                          ))}
                        </div>
                      )
                    }
                    <div className="flex items-center gap-1 ml-1">
                      <Dices size={10} className="text-saga-dim/50" />
...
function Checkbox({ checked, onChange, label }: { checked: boolean; onChange: () => void; label?: string }) {
  return (
    <div className="flex items-center gap-2">
      ...
      {label && <span className="text-[11px] text-saga-muted">{label}</span>}
    </div>
  )
}
    ...
    <div className="space-y-5">
      ...
      <div className="rounded p-4" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)' }}>
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            {canEdit && <button onClick={() => void setStress(stress - 1)} className="w-5 h-5 rounded text-saga-dim hover:text-gold">−</button>}
          ...
          {Array.from({ length: MAX_STRESS }).map((_, i) => (
            <button key={i} onClick={() => canEdit && void setStress(i < stress ? i : i + 1)}
              ...
              style={{
                ...
                borderColor: i < stress ? (stress >= MAX_STRESS ? '#ef4444' : '#c9a22a') : 'rgba(255,255,255,0.2)',
                cursor: canEdit ? 'pointer' : 'default',
              }} />
          ))}
        </div>
        ...
        <div className="mt-3 pt-3" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
          <p className="font-almendra text-[9px] uppercase tracking-widest text-saga-dim mb-2">Condições de Trauma</p>
          {canEdit ? (
            <input
              type="text"
            ...
            <p className="text-sm text-saga-text px-2">{traumaField?.value || <span className="text-saga-dim italic text-xs">—</span>}</p>
          )}
        </div>
      </div>
      ...
      {/* Harm */}
      <div className="rounded p-4" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)' }}>
        <p className="font-almendra text-[9px] uppercase tracking-widest text-saga-dim mb-3">Dano (Harm)</p>
            ...
            return (
              <div key={level} className="flex items-start gap-2 py-1.5 px-2 rounded"
                ...
                <div className="flex-1 space-y-1">
                  <p className="text-[9px] text-saga-dim/60">{label}</p>
                  {keys.map(k => {
                    ...
                    return canEdit ? (
                      <input key={k} type="text" defaultValue={f?.value ?? ''}
                        ...
                        className="w-full bg-transparent text-sm focus:outline-none placeholder:text-saga-dim/40"
      ...
      <div className="grid grid-cols-2 gap-4">
        {/* Armadura */}
        <div className="rounded p-3" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)' }}>
          <p className="font-almendra text-[9px] uppercase tracking-widest text-saga-dim mb-3">Armadura</p>
        ...
        {/* Carga */}
        <div className="rounded p-3" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)' }}>
          <p className="font-almendra text-[9px] uppercase tracking-widest text-saga-dim mb-3">Carga</p>
          <div className="space-y-1.5">
            {['Discreta (1–3)', 'Normal (4–5)', 'Pesada (6)'].map(opt => {
              ...
              return (
                <button key={key} onClick={() => canEdit && void setLoad(key)}
                  ...
                  style={{
                    background: isActive ? 'rgba(201,162,42,0.1)' : 'rgba(255,255,255,0.02)',
                    border: `1px solid ${isActive ? 'rgba(201,162,42,0.4)' : 'rgba(255,255,255,0.05)'}`,
      ...
      {FIELDS.map(f => {
        ...
        return (
          <div key={f.key} className="rounded p-3"
            style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
            <p className="font-almendra text-[9px] uppercase tracking-widest text-saga-dim mb-2">{f.label}</p>
            {canEdit ? (
              f.multi
                ? <textarea
              ...
              <p className="text-sm text-saga-text px-2 py-1 whitespace-pre-wrap">
                {field?.value || <span className="text-saga-dim italic text-xs">—</span>}
              </p>
            )}
          </div>
        )
