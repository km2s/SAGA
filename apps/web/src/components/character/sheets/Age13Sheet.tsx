...
function EditableVal({ attrId, value, characterId, onSaved, className }: { attrId: string; value: number; characterId: string; onSaved: () => void; className?: string }) {
  ...
  if (editing) return (
    <input autoFocus type="number" value={val} onChange={e => setVal(e.target.value)}
      ...
      className={`bg-parchment/60 border border-blue-600/40 rounded text-center font-bold focus:outline-none ${className ?? 'w-14 text-xl'}`} />
  )
  return (
    <span className={`cursor-pointer hover:text-blue-400 font-mono font-bold text-ink transition-colors ${className ?? 'text-xl'}`}
...
function TFField({ characterId, textFields, tfKey, label, placeholder, multiline = false, canEdit, onRefresh, rows = 3 }: {
  ...
  const cls = 'w-full bg-parchment/40 border border-ink/15 rounded-lg text-sm text-ink-soft placeholder-ink-soft/40 focus:outline-none focus:border-blue-500/50 focus:bg-parchment/60 px-3 py-2 transition-colors'
  return (
...
function IconRelationship({ num, characterId, textFields, canEdit, onRefresh }: { num: 1 | 2 | 3; characterId: string; textFields: TextField[]; canEdit: boolean; onRefresh: () => void }) {
  ...
  return (
    <div className="p-3 rounded-lg space-y-2" style={{ background: 'rgba(51,41,29,0.08)', border: `1px solid ${typeColor}30` }}>
      ...
      <div className="space-y-1">
        <label className="text-[10px] font-bold text-ink-soft uppercase tracking-wider">Tipo de Relacionamento</label>
    ...
    <div className="space-y-4">
      ...
      {otAttr && (
        <div className="rounded-xl px-4 py-3" style={{ background: `${ACCENT}15`, border: `1px solid ${ACCENT}30` }}>
          ...
          <p className="text-sm text-ink italic">{textFields.find(f => f.key === 'one_unique_thing')?.value || 'Não definido'}</p>
        </div>
      )}
      ...
      <div className="rounded-xl px-4 py-3 flex items-center justify-between" style={{ background: 'rgba(247,239,221,0.92)', border: '1px solid rgba(51,41,29,0.14)' }}>
        ...
        <div className="flex items-center gap-4 text-xs text-ink-soft">
          {caAttr && <span>CA: <span className="font-bold text-ink">{caAttr.value}</span></span>}
          {dmAttr && <span>DM: <span className="font-bold text-ink">{dmAttr.value}</span></span>}
          {dpAttr && <span>DP: <span className="font-bold text-ink">{dpAttr.value}</span></span>}
        </div>
      </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {stats.map(a => (
              <div key={a.id} className="text-center p-3 rounded-lg" style={{ background: 'rgba(51,41,29,0.08)' }}>
                ...
                <div className="text-[9px] text-ink-soft">mod + nível {characterLevel > 0 ? `(${mod(a.value + characterLevel)})` : ''}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className={card} style={cardStyle}>
            ...
            <div className="grid grid-cols-2 gap-4">
              {pvAttr && (
                <div className="space-y-1">
                  <div className="text-[10px] font-bold text-ink-soft uppercase">Pontos de Vida</div>
              ...
              {recAttr && (
                <div className="space-y-1">
                  <div className="text-[10px] font-bold text-ink-soft uppercase">Recuperações</div>
      ...
      {tab === 'icons' && (
        <div className="space-y-4">
          <p className="text-[11px] text-ink-soft px-1">Relacionamentos com os 13 Ícones — role no início de cada sessão.</p>
