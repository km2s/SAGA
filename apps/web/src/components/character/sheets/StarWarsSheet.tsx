    ...
    <div className="rounded p-4" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)' }}>
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-2">
          {canEdit && <button onClick={() => void adjust(-1)} className="w-5 h-5 text-saga-dim hover:text-saga-text">−</button>}
...
function PericiasTab({ attributes, characterId, canEdit, onRefresh }: {
  ...
  return (
    <div className="space-y-5">
      ...
      {Object.keys(grouped).length === 0 && <p className="text-sm text-saga-dim text-center py-8">Nenhuma perícia cadastrada.</p>}
    </div>
  )
}
