...
function TripleValue({ attr, characterId, canEdit, onSaved }: {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="text-xs text-saga-dim">{half}</span>
      <span className="text-xs text-saga-dim">{fifth}</span>
    </div>
  )
}
    ...
    <div className="space-y-5">
      <div>
        ...
        <div className="rounded overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.07)' }}>
          <div className="grid grid-cols-4 px-3 py-1.5 text-[9px] font-almendra uppercase tracking-widest text-saga-dim"
          ...
          {chars.map(a => (
            <div key={a.id} className="grid grid-cols-4 px-3 py-2 items-center"
              ...
              <span className="text-center text-xs text-saga-dim">{Math.floor(a.value / 2)}</span>
              <span className="text-center text-xs text-saga-dim">{Math.floor(a.value / 5)}</span>
            </div>
          ))}
        </div>
        {damageBonus !== null && (
          <div className="mt-2 px-3 py-2 rounded text-sm" style={{ background: `${ACCENT}10`, border: `1px solid ${ACCENT}30` }}>
            <span className="text-saga-dim text-xs">Bônus de Dano (FOR+TAM={damageBonus}): </span>
              ...
              <div key={a.id} className="flex items-center justify-between px-3 py-2 rounded"
                ...
                {isSan && pod ? (
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-saga-dim">/ {pod.value}</span>
                  </div>
                ) : (
                  <NumericAttr attr={a} characterId={characterId} canEdit={canEdit} onSaved={onRefresh} />
                )}
    ...
    <div className="space-y-5">
      {Object.entries(grouped).map(([cat, attrs]) => (
        <div key={cat}>
          ...
          <div className="rounded overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.07)' }}>
            <div className="grid grid-cols-4 px-3 py-1 text-[9px] font-almendra uppercase tracking-widest text-saga-dim"
              style={{ background: 'rgba(0,0,0,0.2)' }}>
      ...
      {skills.length === 0 && <p className="text-sm text-saga-dim text-center py-8">Nenhuma perícia encontrada.</p>}
    </div>
  )
}
    ...
    <div className="space-y-5">
      {pvAttr && (
        <div className="rounded p-4" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)' }}>
          <p className="font-almendra text-[9px] uppercase tracking-widest text-saga-dim mb-3">Pontos de Vida</p>
      ...
      <div>
        ...
        <div className="space-y-1">
          ...
          {combat.length === 0 && <p className="text-sm text-saga-dim text-center py-4">Nenhuma perícia de combate cadastrada.</p>}
        </div>
      </div>
      ...
      <div className="rounded p-3 text-sm text-saga-dim" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
        <p className="font-almendra text-[9px] uppercase tracking-widest mb-2">Dados de Bônus/Penalidade</p>
        <p>Dado de Bônus: role 2× e fique com a dezena <span className="text-saga-text font-bold">mais baixa</span>.</p>
        <p>Dado de Penalidade: role 2× e fique com a dezena <span className="text-saga-text font-bold">mais alta</span>.</p>
      </div>
    </div>
  )
}
