...
export function NPCHPEditor({ campaignId, npcId, hp, maxHp }: {
  ...
  const hpTextColor = hpPercent > 60 ? 'text-saga-success' : hpPercent > 30 ? 'text-saga-warning' : 'text-saga-danger'
      ...
      <div className="flex items-baseline gap-1.5 mb-3">
        {editing ? (
          <input
            ...
            autoFocus
            className={`w-20 bg-surface-2 border border-gold/40 rounded px-2 py-1 font-cinzel text-3xl font-bold ${hpTextColor} focus:outline-none`}
          />
        ) : (
