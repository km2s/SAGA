      ...
      <div className="p-3">
                ...
                <div key={m.id} className="space-y-1">
                  <div
                    className={`flex items-center justify-between px-3 py-2 rounded transition-all border ${
                      hasLive ? 'bg-emerald-500/[0.08] border-emerald-500/25' : 'bg-ink/[0.03] border-bg/60'
                    }`}>
                    <div className="flex items-center gap-2">
                      {hasLive
                    ...
                    <div className="flex items-center gap-2">
                      {hasLive && otherTokens.length > 0 && (
                        <button
                          ...
                          className={`text-[9px] px-1.5 py-0.5 rounded transition-all border ${isExpanded ? 'text-emerald-400 border-emerald-400/30 bg-emerald-400/10' : 'text-saga-dim border-bg/60 hover:text-emerald-400 hover:border-emerald-400/20'}`}>
                          Tokens
                        </button>
                      )}
        ...
        {liveMembers.length > 0 && (
          <button
            ...
            className="mt-3 w-full py-1.5 rounded text-[11px] text-saga-dim hover:text-saga-danger transition-colors border border-bg/60 hover:border-saga-danger/30">
            Desativar todos
          </button>
        )}
      </div>
