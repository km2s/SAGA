...
export function DiceBar({
  ...
  return (
    <div data-mesa-tutorial="dice" className="shrink-0 border-t border-ink/20 dark:border-bg/60 bg-bg/35">
      <div className="px-3 pt-2.5 pb-1 flex gap-2">
        <input
          ...
          className="flex-1 rounded px-3 py-2 text-[12px] text-saga-text placeholder:text-saga-dim focus:outline-none focus:border-gold/60 disabled:opacity-40 bg-bg/50 border border-ink/20 dark:border-bg/60 transition-colors"
        />
        <button
          onClick={onSendChat}
          disabled={!activeSessionIsActive || !chatInput.trim() || sendingChat}
          className="px-2.5 rounded text-[11px] font-medium text-saga-dim hover:text-saga-text hover:bg-ink/10 disabled:opacity-30 transition-all shrink-0 bg-bg/50 border border-ink/20 dark:border-bg/60">
          <Send size={13}/>
        </button>
      </div>
          ...
          {DICE.map(die=>{
            ...
            return (
              <button key={die} onClick={()=>onRollDie(die)}
                ...
                } ${rolling?'bg-gold/15 border-gold/45 ring-1 ring-gold/60':'bg-bg/40 border-ink/20 dark:border-bg/60'}`}>
